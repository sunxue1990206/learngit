...
/*
*Compared with the group with time
*
*/
...
$.datepicker._selectDay=function(id, month, year, td){
	var target = $(id);
			...
			if(!showTime){
				if(this._timeComparedError(inst,inst.input)){return false;}//bz0038
				}
...
$.datepicker._timeComparedErrorOverride = $.datepicker._timeComparedError;
$.datepicker._timeComparedError=function(inst,time){
	var group=$.trim(time.attr("group"));
	var timenot=$("[group='"+group+"']").not(time);
	if(typeof(timenot)!="undefined"){
		var thisedate=new Date(inst.selectedYear,inst.selectedMonth,inst.selectedDay,0,0,0,0);
		var todayDate=new Date();
			...
			if(!showTime){
				...
				thisedate.setHours(parseInt($('#' + this._mainDivId).siblings().find('span.fragHours').text()),parseInt($('#' + this._mainDivId).siblings().find('span.fragMinutes').text(),0,0));
				}
		var notdate=timenot.val();
		if($.trim(time.attr("id"))=="end_time"||$.trim(time.attr("vertype"))=="end_time"){
			//if(thisedate>todayDate){$("#"+this._mainDivId).find("#"+this._get(inst, 'dateNotice')).show().html("结束时间不能大于当前时间");  $.timepicker.resize(); return true;}
			if(typeof(notdate)!="undefined"&&notdate!=""){
			//	var stardate = eval('new Date(' + notdate.replace(/\d+(?=-[^-]+$)/,
           //  function (a) { return parseInt(a, 10) - 1; }).match(/\d+/g) + ')');
			//console.log(inst);
		    stardate=$.datepicker.parseDate(this._get(inst, 'dateFormat'), notdate, false);
			//console.log($.datepicker._determineDate(notdate, null));
				//console.log(stardate);
				//console.log(thisedate);
				if(stardate>thisedate){$("#"+this._mainDivId).find("#"+this._get(inst, 'dateNotice')).html("结束时间不能小于开始时间").show(); $.timepicker.resize();return true;}
				}
			}
			
		 if($.trim(time.attr("id"))=="start_time"||$.trim(time.attr("vertype"))=="start_time"){
		   
			/*console.log("todayDate");
			console.log(todayDate.toLocaleString());
	    	console.log("thisedate");
		    console.log(thisedate.toLocaleString());*/
			   
			//if(thisedate>todayDate){$("#"+this._mainDivId).find("#"+this._get(inst, 'dateNotice')).show().html("开始时间不能大于当前时间");  $.timepicker.resize(); return true;}
			if(typeof(notdate)!="undefined"&&notdate!=""){
				//var enddate = eval('new Date(' + notdate.replace(/\d+(?=-[^-]+$)/,  
				//function (a) { return parseInt(a, 10) - 1; }).match(/\d+/g) + ')'); 
				 enddate=$.datepicker.parseDate(this._get(inst, 'dateFormat'), notdate, false);
				if(enddate<thisedate){$("#"+this._mainDivId).find("#"+this._get(inst, 'dateNotice')).html("结束时间不能小于开始时间").show();  $.timepicker.resize(); return true;}
				}
		 }
	 }
	$("#"+this._mainDivId).find("#"+this._get(inst, 'dateNotice')).hide();
	$.timepicker.resize();
	return false;
	}
/**
...
$.datepicker._hideDatepicker = function(input, duration) {
    ...
    if (input === undefined && showTime) {//bz009
        if (inst.input) {
	...
	if($.datepicker._timeComparedError(inst,inst.input)&&showTime){return false;}
	var dateCallback=this._get(inst, 'dateCallback');
	if(typeof(dateCallback)!="undefined"){dateCallback(inst.input);}
    // Hide datepicker
