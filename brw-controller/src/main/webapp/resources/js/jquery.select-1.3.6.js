...
*/
		...
		var click_fun =function(){
				...
				if(ulspace>200){$('#'+selectId+' ul').height(200);$('#'+selectId+' ul').outerHeight(true);}
		...
		.bind("click",function(e){
			jQuery("div.select_cont").slideUp();
			jQuery("div.select_cont").siblings().css({padding:"5px"}).find(".focus_div").removeClass("ibg_231x45").css({border:"1px solid #ddd",padding: "0 12px 0 15px"});
			$('#'+selectId+' h4').addClass("over");
			if($('#'+selectId+' ul').css("display") == 'block'){
				$.fn.clearSelectMenu(selectId,selectZindex);
				return false;
	...
	clearSelectMenu:function(selectId,selectZindex){
		if(selectId != undefined){
			...
			var sobj=$('#'+selectId+' select');
		   if(sobj.attr("fill")==1){
			   
			  if($('#'+selectId+' h4').text()==sobj.find("option").first().val()){
				  var vertype=$('#'+selectId+' select').attr("vertype");
				   $('#'+selectId).parent().parent().find(".notice_input").addClass("error_n").removeClass("warn_n sus_n").show().find("span").html($.fn.getVertype(vertype));
				  }else{$('#'+selectId).parent().parent().find(".notice_input").hide();}
			  }
		}
	},
	getVertype:function(vertype){
		var verWord={
			banks:"请选择银行",
			borrowTypeCode:"请选择产品类型"
			}
		if(typeof(verWord[vertype])!="undefined"){return verWord[vertype];}else{return "";}
		
		},
	setSelectValue:function(sID){
		var content = [];
		$.each($('#'+sID+' option'), function(i){
			content.push("<li class='FixSelectBrowser ws'>"+$(this).text()+"</li>");
		});
...
/*select模拟器*/
		 jQuery("select").not('.datepick-new-year,.datepick-new-month').sSelect();
