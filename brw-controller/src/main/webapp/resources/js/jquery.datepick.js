...
$.extend(Datepick.prototype, {
	...
	_adjustDate: function(id, offset, period) {
		...
		this._updateDatepick(inst);
		 ...
		 currenty=  $(".datepick-new-year").val();
		
	    currentm=parseInt($(".datepick-new-month").val())+1; 

		f1(currenty,currentm);
	...
	_selectMonthYear: function(id, select, period) {
		...
		if(period=='Y'){
			 currentm=parseInt( $(".datepick-new-month").val())+1;
			currenty=parseInt(select.options[select.selectedIndex].value, 10);
					
			
			}else{ 
			 currenty=  $(".datepick-new-year").val();
		
			 currentm=parseInt(select.options[select.selectedIndex].value)+1;  
			...
			for (var col = 0; col < numMonths[1]; col++) {
				...
				for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
					...
					for (var dow = 0; dow < 7; dow++) { // create date picker days
						...
						//if( printDate.getDate()==1){html += '<td class="datepick-days-cell hasdata';}
						//else{html += '<td class="datepick-days-cell';};
						...
						html += '<td class="datepick-days-cell'+
							...
							(unselectable ? printDate.getDate() : '<a>' + printDate.getDate() + '</a>')) 
								...
								//if( printDate.getDate()==1){html += '<span class="ws">￥55,500,000.adfasfdafd00</span>';}
						//else{html += '';};
							
							html += '</td>'; // display for this month
...
(function($) {
	$.datepick.regional['zh-CN'] = {
		...
		currentText: '返回当月', currentStatus: '显示本月',
